...
dataX_cnn = np.load('pureX.npy')
dataY_cnn = np.load('dataY.npy')
num_labels = len(set(dataY_cnn))
onehotY = keras.utils.to_categorical(dataY_cnn,num_labels)

X_train, X_test, y_train, y_test = train_test_split(dataX_cnn, onehotY, test_size=0.1, random_state=42)

####Standardize the data####################
cnn_scaler = StandardScaler()
cnn_scaler.fit(X_train)
X_train = cnn_scaler.transform(X_train)
X_test = cnn_scaler.transform(X_test)

pickle.dump(cnn_scaler, open("cnn_scaler.p", "wb"))

batch_size = 64
epochs = 35
width, height = 48, 48

X_train = X_train.reshape(len(X_train),height,width,1)
X_test = X_test.reshape(len(X_test),height,width,1)
model = Sequential()

model.add(Conv2D(64, kernel_size=(3, 3), activation='relu', input_shape=(width, height, 1), kernel_regularizer=l2(0.01)))
